...
const createCommunityPostComment = async ({
  ...
  try {const { pool } = require("./index"); // Import client from the db setup
  const { v4: uuidv4 } = require("uuid"); // Import uuid for generating UUIDs
  ...
  const createCommunityPostComment = async ({
    ...
    try {
      const SQL = `
        INSERT INTO comments ON CONFLICT (id) DO NOTHING (id, created_by, post_id, community_id, comment, created_at)
