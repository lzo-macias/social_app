...
//   const endpoint = `${import.meta.env.VITE_API_BASE_URL}/communitiespost/${communityId}/posts`;
...
function CreateCommunityPostComponent({ communityId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!imgId && !imageUrl) {
      setError("An image is required. Please upload a file or provide an image URL.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div>
      <button className="btn" onClick={() => setShowForm((prev) => !prev)}>
        {showForm ? "Close Post Form" : "Create New Community Post"}
      </button>
      ...
      {showForm && (
        <form onSubmit={handleSubmit}>
          ...
          <div>
            <button type="button" onClick={() => setUploadMethod("file")}>Upload a File</button>
            {/* <button type="button" onClick={() => setUploadMethod("url")}>Use Image URL</button> */}
          </div>
