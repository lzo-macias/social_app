...
const seedDb = async () => {
  try {
    ...
    const profileImages = fs.existsSync(USER_IMAGE_DIR)
      ? fs.readdirSync(USER_IMAGE_DIR).filter(f => f.endsWith(".jpg"))
      : [];
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const sourcePath = path.join(USER_IMAGE_DIR, imageFile);
      const copiedPath = await copyToUploads(sourcePath, imageFile);
    ...
    for (let i = 0; i < communityThemes.length; i++) {
      const theme = communityThemes[i];
      const admin = users[i];
      const folderName = toFolderName(theme);
      const seedDir = path.join(POST_IMAGE_DIR, folderName);
    
      if (!fs.existsSync(seedDir)) {
        console.warn(`⚠️ Folder missing: ${seedDir}`);
        continue;
      }
    
      const seedImages = fs.readdirSync(seedDir).filter(f => f.endsWith(".jpg"));
      if (seedImages.length === 0) {
        console.warn(`⚠️ No images in: ${seedDir}`);
        continue;
      }
    
      const imageFile = seedImages[Math.floor(Math.random() * seedImages.length)];
      const sourcePath = path.join(seedDir, imageFile);
      const copiedPath = await copyToUploads(sourcePath, imageFile);
    
      if (!copiedPath || !fs.existsSync(copiedPath)) {
        console.warn(`⚠️ Failed to copy image: ${sourcePath}`);
        continue;
      }
      ...
      if (!image) {
        console.warn(`⚠️ Failed to save image metadata: ${imageFile}`);
        continue;
      }
    ...
    const filteredCommunities = communities.filter(Boolean);
    console.log(`✅ Seeded ${filteredCommunities.length} communities`);

    for (const user of users) {
      const shuffled = [...filteredCommunities].sort(() => 0.5 - Math.random());
      const selectedCommunities = shuffled.slice(0, 5);
      for (const community of selectedCommunities) {
        await addUserToCommunity(community.id, user.id);
      }
    ...
    for (const community of filteredCommunities) {
      ...
      for (let img of seedImages) {
        ...
        if (!copiedPath || !fs.existsSync(copiedPath)) {
          console.warn(`⚠️ Skipping missing file: ${copiedPath}`);
          continue;
        }
        ...
        if (
          totalCommunityAndUser === 0 &&
          totalUserOnly === 0 &&
          totalCommunityOnly === 0
        ) break;
      }
...
// Wrap in main to safely control the lifecycle
const main = async () => {
  try {
    await seedDb();
  } catch (err) {
    ...
    // Only end the pool when ALL async ops are done
    await pool.end();
  }
};
