...
router.get("/:communtiyId/:postId/comments", isLoggedIn, async (req, res) => {
  try {
    const { postId, communtiyId} = req.params;
    const communityPostComments = await fetchCommentsByPostCommunity(postId, communtiyId);
...
router.put("/:communityId/:postId/:commentId", isLoggedIn, async (req, res, next) => {
  try {
    ...
    const { comment } = req.body;
    const result = await updateCommunityPostComment({ communityId, postId, commentId, comment });
